...
export default function App(){
  ...
  var stAiFiles = useState([]); var aiFiles=stAiFiles[0], setAiFiles=stAiFiles[1];
  ...
  var stERc = useState(false); var stSelEmailTasks = useState({}); var selEmailTasks=stSelEmailTasks[0], setSelEmailTasks=stSelEmailTasks[1];
  var emailReplyCopied=stERc[0], setEmailReplyCopied=stERc[1];
  ...
  async function sendAI(text){
    if(!text.trim()&&aiFiles.length===0)return;
    var ctx2=aiCtx?"\n\n[Tasks: "+tasks.filter(function(t){return !t.done;}).map(function(t){return '"'+t.title+'"('+t.category+','+t.priority+',due '+t.dueDate+')';}).join("; ")+"]":"";
    var displayText=text+(aiFiles.length>0?" ["+aiFiles.map(function(f){return f.name;}).join(", ")+"]":"");
    var msg={role:"user",content:displayText+ctx2};
    var hist=aiMsgs.concat([msg]);
    setAiMsgs(hist);setAiInput("");setAiLoading(true);
    var filesToClear=aiFiles.slice();
    setAiFiles([]);
    try{
      // Build content blocks with any attached files
      var contentBlocks=[];
      for(var fi=0;fi<filesToClear.length;fi++){
        var f=filesToClear[fi];
        try{
          ...
          if(f.type==="application/pdf"||f.name.match(/\.pdf$/i)){
            ...
            contentBlocks.push({type:"image",source:{type:"base64",media_type:f.type,data:b64.data}});
          } else {
            contentBlocks.push({type:"text",text:"[Attachment: "+f.name+"]"});
          }
        }catch(fe){contentBlocks.push({type:"text",text:"[Attachment: "+f.name+" - could not read]"});}
      }
      contentBlocks.push({type:"text",text:(text||"Please analyse the attached file(s).")+ctx2});
      var msgContent=contentBlocks.length>1?contentBlocks:(text+ctx2);
      var r=await fetch("https://hod-proxy.t-d-haycock.workers.dev",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({model:"claude-haiku-4-5-20251001",max_tokens:1000,system:"You are a specialist assistant for "+(settings.name||"a Head of Science")+" at "+(settings.school||"a UK secondary school")+". Help draft professional communications, teaching documents, department admin and leadership tasks. UK English. Concise and practical. Ready-to-use text.",messages:hist.slice(0,-1).map(function(m){return {role:m.role,content:m.content};}).concat([{role:"user",content:msgContent}])})});
  ...
  async function processEmail(){
    ...
    try{
      ...
      if(raw){
        ...
        // Pre-select all actions by default so user can deselect
        if(result.actions&&result.actions.length>0){
          var sel={};
          result.actions.forEach(function(a,i){sel[i]=true;});
          setSelEmailTasks(sel);
        }
      }
    }catch(e){
    ...
    try{
      var r=await fetch("https://hod-proxy.t-d-haycock.workers.dev",{method:"POST",headers:{"Content-Type":"application/json"},
        body:JSON.stringify({model:"claude-haiku-4-5-20251001",max_tokens:800,
          system:"Generate a UK shopping list from a meal plan. Return ONLY a raw JSON array with no markdown, no code blocks, no explanation. Each item must have name (string) and category (string) fields. Category must be exactly one of: fruit and veg, dairy, meat and fish, bakery, cupboard, frozen, drinks, household, other.",
          messages:[{role:"user",content:"Meals: "+planned.map(function(e){return e[0]+": "+e[1];}).join(", ")+". Family of 4. Return raw JSON array only, no markdown."}]})});
      var data=await r.json();
      var txt=data.content&&data.content.find(function(b){return b.type==="text";});
      if(txt){
        var raw=txt.text.replace(/```json/g,"").replace(/```/g,"").trim();
        var match=raw.match(/\[[\s\S]*\]/);
        if(match){
          var items=JSON.parse(match[0]);
          if(Array.isArray(items)&&items.length>0){
            var newItems=items.map(function(i,idx){return {id:"sp"+Date.now()+idx,name:(i.name||i.item||"item").trim(),category:(i.category||"other").toLowerCase().trim(),recurring:false,done:false};});
            setShoppingList(function(prev){return prev.concat(newItems);});
          }
        }
      }
    }catch(e){console.error("Shopping list error:",e);}
    setMealLoading(false);
  }
  ...
  function card(extra){return Object.assign({background:th.surface,border:"1px solid "+th.border,borderRadius:12,padding:"14px 16px",boxShadow:"0 1px 3px rgba(0,0,0,0.05),0 6px 20px "+th.shadow},extra||{});}
  var isDarkHdr=(th.headerBg!=="FFFFFF"&&th.headerBg!=="#FFFFFF"&&th.headerBg!=="#FAF7F2"&&th.headerBg!=="#F0FBF4"&&th.headerBg!=="#F5F5F0"&&th.headerBg!=="#F4F4F0");
  var hdrBg=isDarkHdr
    ?(appMode==="home"?"linear-gradient(135deg,#042F1A 0%,#065F46 48%,#059669 100%)"
      :appMode==="cal"?"linear-gradient(135deg,#0F172A 0%,#1E293B 50%,#334155 100%)"
      :"linear-gradient(135deg,#090F2A 0%,"+th.headerBg+" 52%,#1D4ED8 100%)")
    :th.headerBg;
  ...
  function renderMarkdown(text){
    ...
    while(i<lines.length){
      var line=lines[i];
      // Blank line → spacer
      if(line.trim()===""){elements.push(React.createElement("div",{key:"sp"+i,style:{height:8}}));i++;continue;}
      // Heading ## or ###
      if(line.match(/^#{1,3} /)){
        var htxt=line.replace(/^#{1,3} /,"");
        elements.push(React.createElement("div",{key:"h"+i,style:{fontWeight:800,fontSize:13,marginTop:8,marginBottom:4,color:"inherit"}},inlineMarkdown(htxt)));
        i++;continue;
      }
      // Bullet list item
      if(line.match(/^[-*•] /)){
        var btxt=line.replace(/^[-*•] /,"");
        elements.push(React.createElement("div",{key:"b"+i,style:{display:"flex",gap:6,marginBottom:2}},
          React.createElement("span",{style:{flexShrink:0,marginTop:2}},"•"),
          React.createElement("span",null,inlineMarkdown(btxt))
        ));
        i++;continue;
      }
      // Numbered list
      if(line.match(/^\d+[\.\)] /)){
        var ntxt=line.replace(/^\d+[\.\)] /,"");
        var num=line.match(/^(\d+)/)[1];
        elements.push(React.createElement("div",{key:"n"+i,style:{display:"flex",gap:6,marginBottom:2}},
          React.createElement("span",{style:{flexShrink:0,minWidth:16,fontWeight:600}},num+"."),
          React.createElement("span",null,inlineMarkdown(ntxt))
        ));
        i++;continue;
      }
      // Regular paragraph
      elements.push(React.createElement("div",{key:"p"+i,style:{marginBottom:4}},inlineMarkdown(line)));
      i++;
    }
    return elements;
  }
  function inlineMarkdown(text){
    ...
    while(remaining.length>0){
      var b=remaining.indexOf("**");
      var it=remaining.indexOf("*");
      if(b===0){
        var end=remaining.indexOf("**",2);
        if(end>0){parts.push(React.createElement("strong",{key:key++},remaining.slice(2,end)));remaining=remaining.slice(end+2);continue;}
      }
      if(it===0&&b!==0){
        var end2=remaining.indexOf("*",1);
        if(end2>0){parts.push(React.createElement("em",{key:key++},remaining.slice(1,end2)));remaining=remaining.slice(end2+1);continue;}
      }
      // Find next marker
      var next=Math.min(b<0?99999:b,it<0?99999:it);
      if(next===99999||next>0){
        var chunk=next===99999?remaining:remaining.slice(0,next);
        parts.push(chunk);
        remaining=next===99999?"":remaining.slice(next);
      } else {
        parts.push(remaining[0]);
        remaining=remaining.slice(1);
      }
    }
    return parts.length===1&&typeof parts[0]==="string"?parts[0]:React.createElement("span",{key:"il"},parts);
  }
  ...
  return (
    <div className={appMode==="home"?"home-mode":""} style={{minHeight:"100vh",background:th.bg,fontFamily:"'DM Mono','Fira Mono',monospace",color:th.text}}>
      ...
      {/* Header */}
      <div style={{background:hdrBg,borderBottom:"1px solid "+(isDarkHdr?"rgba(255,255,255,0.1)":th.headerBorder),padding:"16px 16px 12px",position:"sticky",top:0,zIndex:100,boxShadow:"0 4px 24px rgba(0,0,0,0.18)"}}>
        <div style={{maxWidth:940,margin:"0 auto"}}>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:12,flexWrap:"wrap",gap:8}}>
            <div>
              ...
              <div style={{display:"flex",justifyContent:"center",marginTop:6}}>
                <div style={{display:"flex",background:"rgba(255,255,255,0.12)",borderRadius:22,padding:4,gap:3,boxShadow:"inset 0 1px 0 rgba(255,255,255,0.15),0 2px 8px rgba(0,0,0,0.2)",backdropFilter:"blur(8px)"}}>
                  <button onClick={function(){switchMode("work");}} style={{background:appMode==="work"?"white":"transparent",color:appMode==="work"?"#1E3A8A":"rgba(255,255,255,0.8)",border:"none",borderRadius:18,padding:"5px 13px",fontSize:11,fontFamily:"'Syne',sans-serif",fontWeight:700,cursor:"pointer",transition:"all 0.2s",letterSpacing:"0.05em",boxShadow:appMode==="work"?"0 2px 10px rgba(0,0,0,0.25)":"none"}}>&#128188; Work</button>
                  <button onClick={function(){switchMode("cal");}} style={{background:appMode==="cal"?"white":"transparent",color:appMode==="cal"?"#1E293B":"rgba(255,255,255,0.8)",border:"none",borderRadius:18,padding:"5px 13px",fontSize:10,fontFamily:"'Syne',sans-serif",fontWeight:700,cursor:"pointer",transition:"all 0.2s",boxShadow:appMode==="cal"?"0 2px 10px rgba(0,0,0,0.25)":"none"}}>&#128197; Calendar</button>
                  <button onClick={function(){switchMode("home");}} style={{background:appMode==="home"?"white":"transparent",color:appMode==="home"?"#1A3A08":"rgba(255,255,255,0.8)",border:"none",borderRadius:18,padding:"5px 13px",fontSize:11,fontFamily:"'Syne',sans-serif",fontWeight:700,cursor:"pointer",transition:"all 0.2s",letterSpacing:"0.05em",boxShadow:appMode==="home"?"0 2px 10px rgba(0,0,0,0.25)":"none"}}>&#127968; Home</button>
                </div>
              </div>
              <div style={{fontFamily:"'Syne',sans-serif",fontSize:24,fontWeight:800,color:th.headerText,letterSpacing:"-0.03em",textShadow:isDarkHdr?"0 1px 8px rgba(0,0,0,0.3)":"none"}}>{(function(){var d=new Date();var days=["Sunday","Monday","Tuesday","Wednesday","Thursday","Friday","Saturday"];var months=["January","February","March","April","May","June","July","August","September","October","November","December"];return days[d.getDay()]+", "+d.getDate()+" "+months[d.getMonth()]+" "+d.getFullYear();})()}</div>
            </div>
            <div style={{display:"flex",gap:6,flexWrap:"wrap",justifyContent:"flex-end"}}>
              {[
                {n:pending.length,sub:"pending",bg:pending.length>0?th.accent:th.navActive+"22",col:pending.length>0?"#FFFFFF":th.navActive||th.accent,strong:pending.length>0},
                ...
                return React.createElement("div",{key:i,style:{background:s.bg,border:"1px solid "+s.col+(s.strong?"":"44"),borderRadius:10,padding:"6px 13px",textAlign:"center",minWidth:54,boxShadow:s.strong?"0 2px 12px "+s.col+"66":"none"}},
                  React.createElement("div",{style:{fontSize:18,fontWeight:800,color:s.col,fontFamily:"'Syne',sans-serif",lineHeight:1}},s.n),
          ...
          {appMode!=="cal"&&<div style={{display:"flex",gap:4,flexWrap:"wrap"}}>
            {NAV.map(function(v){
              return (
                <button key={v.id} className="nb" onClick={function(){setView(v.id);}} style={{background:view===v.id?th.navActive:th.navInactive,color:view===v.id?th.navActiveTxt:th.navInactiveTxt,border:"1px solid "+(view===v.id?th.navActive:th.navBorder),borderRadius:8,padding:"5px 11px",fontSize:10,fontFamily:"'DM Mono',monospace",fontWeight:view===v.id?600:400,letterSpacing:"0.04em",boxShadow:view===v.id?"0 2px 8px rgba(0,0,0,0.15)":"none",transition:"all 0.15s"}}>{v.label}</button>
              );
            })}
          </div>}
        </div>
        ...
        {appMode==="work"&&view==="ai"&&(
          <div className="fu" style={{display:"flex",flexDirection:"column",height:"calc(100vh - 200px)",minHeight:420}}>
              ...
              {aiMsgs.map(function(msg,i){return(
                <div key={i} style={{marginBottom:14,display:"flex",gap:9,flexDirection:msg.role==="user"?"row-reverse":"row"}} className="fu">
                  ...
                  <div style={{maxWidth:"80%",background:msg.role==="user"?th.accentSub:th.surface2,border:"1px solid "+(msg.role==="user"?th.accent+"33":th.border),borderRadius:msg.role==="user"?"10px 2px 10px 10px":"2px 10px 10px 10px",padding:"10px 13px"}}>
                    <div style={{fontSize:12,color:th.text,lineHeight:1.65}}>
                      {msg.role==="assistant"?renderMarkdown(msg.content):msg.content}
                    </div>
                    {msg.role==="assistant"&&(
                      <button onClick={function(){
                        var clean=stripMarkdown(msg.content);
                        if(navigator.clipboard){navigator.clipboard.writeText(clean);}
                        else{var ta=document.createElement("textarea");ta.value=clean;document.body.appendChild(ta);ta.select();document.execCommand("copy");document.body.removeChild(ta);}
                      }} style={{marginTop:6,background:"none",border:"1px solid "+th.border,borderRadius:5,padding:"3px 10px",fontSize:10,color:th.textMuted,cursor:"pointer",fontFamily:"inherit",display:"block"}}>Copy plain text</button>
                    )}
                  </div>
                </div>
              );})}
            ...
            <div style={{display:"flex",gap:8}}>
              {/* File previews */}
              {aiFiles.length>0&&(
                <div style={{display:"flex",flexWrap:"wrap",gap:4,padding:"4px 0",width:"100%"}}>
                  {aiFiles.map(function(f,i){return(
                    <div key={i} style={{display:"flex",alignItems:"center",gap:4,background:th.accentSub,border:"1px solid "+th.accent+"33",borderRadius:12,padding:"2px 8px",fontSize:11,color:th.accent}}>
                      <span>{f.type.startsWith("image/")?"🖼️":"📄"}</span>
                      <span style={{maxWidth:120,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{f.name}</span>
                      <button onClick={function(){setAiFiles(function(p){return p.filter(function(_,j){return j!==i;});});}} style={{background:"none",border:"none",cursor:"pointer",color:th.accent,fontSize:12,padding:"0 2px",lineHeight:1}}>×</button>
                    </div>
                  );})}
                </div>
              )}
              <div style={{display:"flex",gap:6,width:"100%",alignItems:"flex-end"}}>
                ...
                <div style={{display:"flex",flexDirection:"column",gap:4}}>
                  <label style={{cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",width:36,height:36,background:th.surface2,border:"1px solid "+th.border,borderRadius:7,fontSize:16}} title="Attach photo or file">
                    📎
                    <input type="file" accept="image/*,.pdf,.doc,.docx,.txt" multiple onChange={function(e){var newFiles=Array.from(e.target.files);setAiFiles(function(p){return p.concat(newFiles);});e.target.value="";}} style={{display:"none"}}/>
                  </label>
                  {MicBtn(setAiInput,"ai-input")}
                </div>
              </div>
              <button onClick={function(){sendAI(aiInput);}} disabled={(!aiInput.trim()&&aiFiles.length===0)||aiLoading} style={{background:(aiInput.trim()||aiFiles.length>0)&&!aiLoading?th.accent:th.surface2,color:aiInput.trim()&&!aiLoading?th.accentText:th.textMuted,border:"none",borderRadius:8,padding:"0 16px",fontFamily:"'Syne',sans-serif",fontWeight:700,fontSize:12,cursor:aiInput.trim()&&!aiLoading?"pointer":"default",minWidth:55}}>{aiLoading?"...":"Send"}</button>
            </div>
          </div>
        )}
                ...
                {emailResult.actions&&emailResult.actions.length>0&&(
                  <div style={card({marginBottom:12})}>
                    <div style={{fontSize:10,fontWeight:700,color:th.textMuted,letterSpacing:"0.09em",textTransform:"uppercase",marginBottom:10}}>
                      Action items
                      <span style={{marginLeft:8,fontSize:9,background:th.accentSub,color:th.accent,borderRadius:3,padding:"1px 6px",border:"1px solid "+th.accent+"44"}}>{Object.values(selEmailTasks).filter(Boolean).length+" of "+emailResult.actions.length+" selected"}</span>
                    </div>
                        ...
                        <div key={i} style={{background:th.surface2,border:"1px solid "+th.border,borderLeft:"3px solid "+cCat.color,borderRadius:8,padding:"10px 12px",marginBottom:7,display:"flex",justifyContent:"space-between",alignItems:"flex-start",gap:10,flexWrap:"wrap"}}>
                          <div style={{flex:1,minWidth:0}}>
                            ...
                            <div style={{display:"flex",gap:6,flexWrap:"wrap"}}>
                              ...
                              {selEmailTasks[i]&&<span style={{fontSize:9,background:th.greenBg,color:th.green,borderRadius:3,padding:"1px 5px"}}>✓ Selected</span>}
                            </div>
                            {action.notes&&<div style={{fontSize:11,color:th.textMuted,marginTop:4,fontStyle:"italic"}}>{action.notes}</div>}
                          </div>
                          <div style={{display:"flex",flexDirection:"column",gap:5,flexShrink:0,alignItems:"flex-end"}}>
                            <button onClick={function(){setSelEmailTasks(function(s){var n=Object.assign({},s);n[i]=!s[i];return n;});}} style={{background:selEmailTasks[i]?th.accent:th.surface2,color:selEmailTasks[i]?th.accentText:th.textSub,border:"1.5px solid "+(selEmailTasks[i]?th.accent:th.border),borderRadius:6,padding:"4px 10px",fontSize:10,cursor:"pointer",fontFamily:"inherit",fontWeight:700,whiteSpace:"nowrap"}}>{selEmailTasks[i]?"✓ Add to tasks":"+ Add to tasks"}</button>
                            <button onClick={function(){setAddToMtgPoint(action.point);setView("meetings");}} style={{background:"none",border:"1px solid #8B5CF655",borderRadius:6,padding:"4px 8px",fontSize:10,color:"#8B5CF6",cursor:"pointer",fontFamily:"inherit",fontWeight:600,whiteSpace:"nowrap"}}>+ Agenda</button>
                          </div>
                        </div>
                      );
                    })}
                    {Object.values(selEmailTasks).some(Boolean)&&(
                      <button onClick={function(){
                        var newTasks=emailResult.actions.filter(function(a,i){return selEmailTasks[i]&&a.title;}).map(function(a){
                        ...
                        setSelEmailTasks({});
                      }} style={{width:"100%",background:th.accent,color:th.accentText,border:"none",borderRadius:8,padding:"10px 0",fontFamily:"'Syne',sans-serif",fontWeight:700,fontSize:12,cursor:"pointer",marginTop:6}}>{"Add "+Object.values(selEmailTasks).filter(Boolean).length+" task"+(Object.values(selEmailTasks).filter(Boolean).length===1?"":"s")+" to task board"}</button>
                    )}
                  </div>
                )}
        ...
        {appMode==="home"&&view==="h-clubs"&&(
          <div className="fu">
            ...
            <div style={card({marginBottom:14})}>
              ...
              <div style={{display:"grid",gridTemplateColumns:"repeat(7,minmax(52px,1fr))",gap:4,minWidth:380}}>
